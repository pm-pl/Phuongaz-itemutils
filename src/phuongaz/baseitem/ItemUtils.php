...
class ItemUtils {
    ...
    public static function fromArray(array $item) :?Item {
        ...
        if(is_numeric($itemExplode[0])) {
            $itemObject = ItemFactory::getInstance()->get((int)$itemExplode[0], $itemExplode[1] ?? 0, $itemExplode[2] ?? 1);
        }else {
            $itemObject = StringToItemParser::getInstance()->parse($itemExplode[0]);
            if($itemObject != null) {
                $itemObject = $itemObject->setCount($itemExplode[2] ?? 1);
            }
        }
        if(isset($item['name'])) {
        ...
        return $itemObject instanceof Item ? $itemObject : null;
    }
